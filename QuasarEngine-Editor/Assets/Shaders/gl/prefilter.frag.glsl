...
void main()
{
    ...
    for (uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        vec3 H  = ImportanceSampleGGX(Xi, N, object_ubo.roughness);
        vec3 L  = normalize(2.0 * dot(V, H) * H - V);

        float NdotL = max(dot(N, L), 0.0);
        if (NdotL > 0.0)
        {
            float D     = DistributionGGX(N, H, object_ubo.roughness);
            float NdotH = max(dot(N, H), 0.0);
            float HdotV = max(dot(H, V), 0.0);
            float pdf   = D * NdotH / (4.0 * HdotV) + 1e-4;

            float saSample = 1.0 / (float(SAMPLE_COUNT) * pdf + 1e-4);
            float mipLevel = (object_ubo.roughness == 0.0) ? 0.0 : 0.5 * log2(saSample / saTexel);
